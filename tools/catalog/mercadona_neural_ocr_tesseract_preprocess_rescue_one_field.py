...
ONE_FIELD_RESCUE_POLICY_VERSION = "1.0.0"
CORE_FIELDS = rescue.CORE_FIELDS


def _should_run_one_field_preprocess_rescue(ensemble) -> bool:
    """Spend bounded correlated Tesseract passes on safe one-to-three-field rows.

    This broadens only the observation trigger to include rows with a single
    already-corroborated core field. The final acceptance contract is unchanged:
    ...
    return bool(
        ...
        and ensemble.corroborated_fields in {1, 2, 3}
...
def _rewrite_summary_one_field() -> None:
    ...
    payload["mode"] = "PADDLEOCR_TESSERACT_WITH_SAFE_ONE_TO_THREE_FIELD_TESSERACT_PREPROCESS_RESCUE"
    payload["rescue_policy_version"] = ONE_FIELD_RESCUE_POLICY_VERSION
    payload["preprocess_trigger_corroborated_fields"] = [1, 2, 3]
...
def main() -> int:
    rescue._should_run_preprocess_rescue = _should_run_one_field_preprocess_rescue
    rescue._rewrite_summary = _rewrite_summary_one_field
    return rescue.main()
