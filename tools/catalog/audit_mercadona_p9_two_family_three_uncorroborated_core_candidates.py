...
def candidate_payload(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if len(field_rows) != 4 or len(uncorroborated) != 3:
        return None
...
def main() -> int:
    ...
    field_triplet_counts = Counter("+".join(row["uncorroborated_core_fields"]) for row in candidates)
    ...
    summary = {
        ...
        "candidate_policy": (
            "stable p9 original+replay REVIEW + all 4 core values observed + explicit 100g/100ml "
            "basis + exactly 2 independent OCR families + exactly 1 corroborated core field + "
            "exactly 3 uncorroborated core fields each observed by exactly 1 family + no OCR hard "
            "conflict + no energy/macro mismatch; routing only"
        ),
        "uncorroborated_core_triplet_counts": dict(sorted(field_triplet_counts.items())),
        ...
        "next_safe_experiment": (
            ...
            "unchanged and requires all four core fields independently corroborated"
        ),
    ...
    (out / "two-family-three-uncorroborated-core-candidates.jsonl").write_text(
