...
def candidate_payload(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if families != 2 or corroborated != 3:
        return None
    if "UNCORROBORATED_BASIS" in reasons:
        return None
    ...
    uncorroborated = [str(field.get("name")) for field in field_rows if not bool(field.get("corroborated"))]
    ...
    return {
        ...
        "uncorroborated_core_field": uncorroborated[0],
...
def main() -> int:
    ...
    perspective_counts = Counter(str(row.get("perspective")) for row in candidates)
    missing_field_counts = Counter(str(row["uncorroborated_core_field"]) for row in candidates)
    ...
    summary = {
        ...
        "candidate_policy": (
            "stable original+replay REVIEW + all 4 core values observed + safely explicit 100g/100ml basis + "
            "exactly 2 independent OCR families + exactly 3 corroborated core fields + exactly 1 uncorroborated "
            "core field + no OCR hard conflict + no energy/macro mismatch; routing only, never usable nutrition"
        ),
        "perspective_counts": dict(sorted(perspective_counts.items())),
        "uncorroborated_core_field_counts": dict(sorted(missing_field_counts.items())),
        "reason_prefix_counts": dict(sorted(reason_counts.items())),
        "next_safe_experiment": (
            "bounded Tesseract CLAHE/OTSU/adaptive preprocessing on a deterministic pilot; final DECLARED "
            "acceptance is unchanged and requires independent-family corroboration of all four core fields"
        ),
    ...
    (out / "one-uncorroborated-core-candidates.jsonl").write_text(
