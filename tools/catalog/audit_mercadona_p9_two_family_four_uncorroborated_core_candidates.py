...
def candidate_payload(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if basis not in {"100_g", "100_ml"} or "UNCORROBORATED_BASIS" in reasons:
        return None
    if families != 2 or corroborated != 0:
        return None
    ...
    if len(field_rows) != 4 or len(uncorroborated) != 4:
        return None
...
def main() -> int:
    ...
    summary = {
        ...
        "candidate_policy": (
            ...
            "basis + exactly 2 independent OCR families + zero corroborated core fields + exactly 4 "
            "uncorroborated core fields each observed by exactly 1 family + no OCR hard conflict + "
            "no energy/macro mismatch; routing only"
        ),
        "uncorroborated_source_family_counts": dict(sorted(source_counts.items())),
        "next_safe_experiment": (
            "bounded full 3-product Tesseract CLAHE/OTSU/adaptive pilot; acceptance unchanged and "
            "requires all four core fields independently corroborated"
        ),
    ...
    (out / "two-family-four-uncorroborated-core-candidates.jsonl").write_text(
