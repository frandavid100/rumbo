...
def main() -> int:
    ...
    # recursively discovered first-party category node exhausted its own pageToken chain
    # without API errors or unaddressable children, followed by global UUID deduplication.
    complete = (
        bool(checks)
        and not failed
    ...
    report = {
        "child_shards_seen": len(checks),
        "child_shards_failed": len(failed),
        "failed": failed,
        ...
        "completeness_basis": "recursive_first_party_category_tree_plus_pageToken_exhaustion_then_uuid_dedup",
