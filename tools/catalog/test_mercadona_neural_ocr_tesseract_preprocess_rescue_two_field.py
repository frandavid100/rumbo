...
from mercadona_neural_ocr_tesseract_preprocess_rescue_two_field import (
    _should_run_two_field_preprocess_rescue,
)
...
class TwoFieldPreprocessRescuePolicyTests(unittest.TestCase):
    def test_accepts_safe_two_field_shape(self):
        self.assertTrue(_should_run_two_field_preprocess_rescue(FakeEnsemble()))

    def test_keeps_validated_three_field_shape(self):
        self.assertTrue(
            _should_run_two_field_preprocess_rescue(FakeEnsemble(corroborated_fields=3))
        )

    def test_rejects_only_one_corroborated_field(self):
        self.assertFalse(
            _should_run_two_field_preprocess_rescue(FakeEnsemble(corroborated_fields=1))
        )

    def test_rejects_already_declared(self):
        self.assertFalse(
            _should_run_two_field_preprocess_rescue(FakeEnsemble(declared_usable=True))
        )

    def test_rejects_missing_basis(self):
        self.assertFalse(_should_run_two_field_preprocess_rescue(FakeEnsemble(basis=None)))
    ...
    def test_rejects_missing_core_value(self):
        ...
        self.assertFalse(_should_run_two_field_preprocess_rescue(ensemble))
