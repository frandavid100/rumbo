...
from mercadona_label_pipeline import download_label_image, process_label_file
...
def main() -> int:
    report = {"products_requested": len(PRODUCTS), "api_fetched": 0, "with_photos": 0,
              "declared": 0, "review_only": 0, "unreadable": 0, "items": []}
            ...
            with tempfile.TemporaryDirectory(prefix="rumbo-live-label-") as td:
                for evidence in product.label_images:
                    ...
                    try:
                        ...
                        item["attempts"].append({"image_index": evidence.image_index, "psm": None,
                            "status": "DOWNLOAD_ERROR", "reason": f"{type(exc).__name__}:{exc}",
                            "confidence": None})
                        continue
                    for psm in (6, 11):
                        result = process_label_file(evidence, path, gtin=product.ean,
                                                    brand=product.brand, extractor=extractor_for(psm))
                        attempt = {"image_index": evidence.image_index, "psm": psm,
                        ...
                        if result.status == "DECLARED" and result.candidate is not None:
                            ...
                            item["declared"] = {"image_index": evidence.image_index, "psm": psm,
