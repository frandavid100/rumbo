...
from audit_mercadona_p9_two_family_three_uncorroborated_core_candidates import candidate_payload
...
def row(
    *,
    ...
    uncorroborated=("protein_g", "carbohydrate_g", "fat_g"),
...
class CandidatePayloadTests(unittest.TestCase):
    def test_two_family_three_uncorroborated_core_is_candidate(self):
        ...
        self.assertEqual(
            ...
            ["carbohydrate_g", "fat_g", "protein_g"],
        )
        self.assertEqual(
            payload["uncorroborated_source_families"],
            {
                "protein_g": "paddleocr",
                "carbohydrate_g": "tesseract",
                "fat_g": "paddleocr",
        ...
        self.assertEqual(payload["corroborated_fields"], 1)
    ...
    def test_previous_two_field_short_stratum_is_excluded(self):
        self.assertIsNone(
            candidate_payload(
                row(corroborated=2, uncorroborated=("carbohydrate_g", "fat_g"))
            )
        )

    def test_three_family_stratum_is_excluded(self):
        self.assertIsNone(candidate_payload(row(families=3)))

    def test_wrong_corroborated_count_is_excluded(self):
        self.assertIsNone(candidate_payload(row(corroborated=0)))
    ...
    def test_exactly_three_uncorroborated_fields_are_required(self):
        self.assertIsNone(
            candidate_payload(
                row(uncorroborated=("carbohydrate_g", "fat_g"), corroborated=1)
            )
        )
        self.assertIsNone(
            candidate_payload(row(uncorroborated=tuple(CORE.keys()), corroborated=1))
        )
