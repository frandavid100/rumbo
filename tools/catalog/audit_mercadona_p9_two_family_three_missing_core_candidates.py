...
def candidate_payload(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if len(observed_fields) != 1 or len(missing_fields) != 3:
        return None
    ...
    if set(field_rows) != set(observed_fields):
        return None
    observed = observed_fields[0]
    if not bool(field_rows[observed].get("corroborated")):
        return None
    if len(field_rows[observed].get("engine_families") or []) < 2:
        return None
...
def main() -> int:
    ...
    missing_counts = Counter("+".join(row["missing_core_fields"]) for row in candidates)
    observed_counts = Counter("+".join(row["observed_core_fields"]) for row in candidates)
    ...
    (out / "two-family-three-missing-core-candidates.jsonl").write_text(
