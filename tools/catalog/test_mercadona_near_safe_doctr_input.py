...
def diagnostic(pid: str, *, corroborated: int, families: int = 3, blockers=None):
    return {
        "product_id": pid,
        "ean": f"84{pid}",
...
class MercadonaNearSafeDocTRInputTest(unittest.TestCase):
    def test_three_then_two_then_one_of_four_are_prioritized_and_four_of_four_is_excluded(self):
        rows = [
            diagnostic("100", corroborated=1),
        ...
        self.assertEqual([row["product_id"] for row in selected], ["300", "200", "100"])
        self.assertEqual(summary["selected_corroboration"], {"300": 3, "200": 2, "100": 1})
        self.assertIn("400", summary["rejected"])
        self.assertIn("NOT_ONE_TO_THREE_OF_FOUR", summary["rejected"]["400"])
        self.assertIn("SAFETY_BLOCKED", summary["rejected"]["500"])
        self.assertFalse(summary["acceptance_policy_changed"])

    def test_one_of_four_requires_two_independent_engine_families(self):
        rows = [
            diagnostic("100", corroborated=1, families=2),
            diagnostic("200", corroborated=1, families=1),
        ]
        selected, summary = select_near_safe_rows(rows, limit=8)
        self.assertEqual([row["product_id"] for row in selected], ["100"])
        self.assertIn("200", summary["rejected"])
        self.assertIn("INSUFFICIENT_ENGINE_FAMILIES", summary["rejected"]["200"])
        self.assertFalse(summary["acceptance_policy_changed"])
    ...
    def test_live_builder_requires_exact_ean_and_exact_current_image(self):
        selected = [
            ...
            diagnostic("200", corroborated=2),
            diagnostic("300", corroborated=1, families=2),
        ]
        details = [
            {
                "product_id": "100",
