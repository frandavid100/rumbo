...
def main() -> None:
    ...
    parser.add_argument(
        "--candidates",
        default="fixtures/carrefour_candidate_urls_radarsuper.jsonl",
        type=Path,
    )
    parser.add_argument(
        "--verified",
        default="fixtures/carrefour_first_party_products_cumulative.jsonl",
        type=Path,
    )
    parser.add_argument(
        "--output",
        default="fixtures/carrefour_unverified_candidates_head.jsonl",
        type=Path,
    )
    parser.add_argument(
        "--summary",
        default="fixtures/carrefour_unverified_candidates_summary.json",
        type=Path,
    )
    ...
    for row in candidate_rows:
        ...
        if not sku:
            missing_sku += 1
            continue
        # First candidate wins so output is deterministic even if the discovery source
        # contains duplicates.
    ...
    for sku, row in by_sku.items():
        ...
        selected = {
            ...
            "provenance_note": (
                "External discovery hint only. Do not attribute any field in this row to "
    ...
    summary = {
        ...
        "counts": {
            ...
            "candidate_unique_skus": len(by_sku),
            "candidate_rows_missing_sku": missing_sku,
            "verified_first_party_unique_skus": len(verified_skus),
            "candidate_skus_already_verified": already_verified,
            "candidate_skus_not_yet_verified": len(unverified),
