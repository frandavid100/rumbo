...
async def main() -> int:
        ...
        async def on_request(req):
            try:
                ...
                events.append({
                    ...
                    "headers": selected_headers(req.headers),
                })
        ...
        async def on_response(resp):
            try:
                ...
                item = {
                    ...
                    "request_headers": selected_headers(req.headers),
                }
                if "json" in ct.lower() or "/api/" in resp.url:
                    try:
                        text = await resp.text()
                        item["body"] = text[:1_000_000]
                        item["body_length"] = len(text)
                    except Exception as exc:
    ...
    # Compact, paired view that preserves the critical request recipe.
    summary = []
    for e in api:
        row = {k: e.get(k) for k in (
            "kind", "method", "url", "status", "post_data", "request_post_data",
            "content_type", "body_length"
        ) if e.get(k) is not None}
        body = e.get("body")
        if body:
            row["body_preview"] = body[:12_000]
    ...
    # Extract unique API recipes for fast inspection in later autonomous runs.
    recipes = []
    seen = set()
    for e in api:
        if e.get("kind") not in ("request", "response"):
            continue
        method = e.get("method")
        url = e.get("url")
        ...
        if key in seen:
            continue
        seen.add(key)
        recipes.append({"method": method, "url": url, "post_data": post})
    (out / "recipes.json").write_text(json.dumps(recipes, ensure_ascii=False, indent=2), encoding="utf-8")

    print(json.dumps(recipes[:120], ensure_ascii=False, indent=2))
    return 0
