...
def candidate_payload(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if basis not in {"100_g", "100_ml"}:
        return None
    if families != 1 or corroborated != 0:
        return None
...
def main() -> int:
    ...
    summary = {
        ...
        "candidate_policy": (
            "stable original+replay REVIEW + all 4 core values observed + explicit 100g/100ml basis + "
            "exactly 1 independent OCR family + 0 corroborated core fields + no OCR hard conflict + "
            "no energy/macro mismatch; this is rescue routing only and never makes nutrition usable"
        ),
        ...
        "next_safe_experiment": (
            ...
            "acceptance remains unchanged and requires independent-family corroboration of all core fields"
        ),
    ...
    (out / "single-family-candidates.jsonl").write_text(
