...
def candidate_payload(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if basis not in {"100_g", "100_ml"} or "UNCORROBORATED_BASIS" in reasons:
        return None
    if families != 3 or corroborated != 2:
        return None
...
def main() -> int:
    ...
    summary = {
        ...
        "candidate_policy": "stable p9 original+replay REVIEW + all 4 core values observed + explicit 100g/100ml basis + exactly 3 independent OCR families + exactly 2 corroborated core fields + exactly 2 uncorroborated core fields each observed by exactly 1 family + no OCR hard conflict + no energy/macro mismatch; routing only",
        ...
        "next_safe_experiment": "bounded 16-product Tesseract CLAHE/OTSU/adaptive pilot; acceptance unchanged and requires all four core fields independently corroborated",
    ...
    (out / "three-family-two-uncorroborated-core-candidates.jsonl").write_text(
