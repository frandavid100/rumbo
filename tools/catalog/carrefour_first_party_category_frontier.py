...
VERSION = "carrefour-first-party-category-frontier-1.0"
...
def fetch_category(url: str, timeout: int, attempts: int) -> tuple[int | None, str, str, str | None]:
    ...
    for attempt in range(max(1, attempts)):
        try:
            ...
            with urlopen(request, timeout=timeout) as response:
                ...
                return response.status, response.geturl(), raw_bytes.decode("utf-8", "replace"), None
        except Exception as exc:
            last = exc
            if attempt + 1 < max(1, attempts):
                time.sleep(0.6 * (attempt + 1))
    return None, url, "", f"{type(last).__name__}:{last}" if last else "UNKNOWN_FETCH_ERROR"
...
def run(args: argparse.Namespace) -> int:
    ...
    while queue:
        ...
        visited.add(url)
        observed_at = now_iso()
        status, final_url, raw, error = fetch_category(url, args.timeout, args.attempts)
        ...
        print(json.dumps({"category": canonical_final, "depth": depth, "status": status, "children": len(children), "products": len(product_urls), "error": error}, ensure_ascii=False))
