...
def candidate_payload(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if basis not in {"100_g", "100_ml"} or "UNCORROBORATED_BASIS" in reasons:
        return None
    if families != 3 or corroborated != 3:
        return None
    ...
    if len(field_rows) != 4 or len(uncorroborated) != 1:
        return None
    source_families = [str(family) for family in (uncorroborated[0].get("engine_families") or [])]
    ...
    return {
        ...
        "uncorroborated_core_field": str(uncorroborated[0].get("name")),
        "uncorroborated_source_family": source_families[0],
...
def main() -> int:
    ...
    summary = {
        ...
        "candidate_policy": "stable p9 original+replay REVIEW + all 4 core values observed + explicit 100g/100ml basis + exactly 3 independent OCR families + exactly 3 corroborated core fields + exactly 1 uncorroborated core field observed by exactly 1 family + no OCR hard conflict + no energy/macro mismatch; routing only",
        "uncorroborated_core_field_counts": dict(sorted(Counter(row["uncorroborated_core_field"] for row in candidates).items())),
        "uncorroborated_source_family_counts": dict(sorted(Counter(row["uncorroborated_source_family"] for row in candidates).items())),
        "next_safe_experiment": "bounded Tesseract CLAHE/OTSU/adaptive pilot; acceptance unchanged and requires all four core fields independently corroborated",
    ...
    (out / "three-family-one-uncorroborated-core-candidates.jsonl").write_text("".join(json.dumps(row, ensure_ascii=False, sort_keys=True) + "\n" for row in candidates), encoding="utf-8")
