...
class MenuConstraintContractTest {
    ...
    @Test
    fun generatorFailureWithoutProofIsSearchInconclusive() {
        val powder = food(
        ...
        assertNull(result.witness)
        assertTrue(result.limitingFactors.any { it.contains("no se ha demostrado", ignoreCase = true) })
    }

    @Test
    fun feasibleAssessmentCarriesReproducibleWitness() {
        val completeMeal = food(
            3, "Comida completa", FoodCategory.OTHER,
    ...
    fun sharedConstraintModelPreservesLegacyRuleFiltering() {
        val active = food(4, "Activo", FoodCategory.OTHER, 1000.0, 80.0, 100.0, 35.0)
        val inactive = active.copy(id = 5, name = "Inactivo")
